...
from config import ERC20_ABI, RPC, TOKENS
from utils.utils import sleep
from settings import GAS_MULTIPLAYER
from utils.exceptions import TransactionNotFound
...
class Account:
    ...
    async def check_allowance(self, token_address: str, contract_address: str):
    ...
    async def get_balance(self, contract_address: str) -> dict:
        contract = self.get_contract(self.w3.to_checksum_address(contract_address))
    ...
    async def get_amount(
        ...
        if from_token == 'ETH':
            ...
            balance = await self.get_balance(TOKENS[from_token])
    ...
    async def wait_until_tx_finished(self, hash: str, max_wait_time = 180):
        ...
        while True:
            try:
                ...
                if status == 1:
                    logger.success(f'{self.account_id} | {self.address} | {self.explorer}{hash.hex()} successfully!')
                    return True
                elif status is None:
                    await asyncio.sleep(0.3)
                else:
                    logger.success(f'{self.account_id} | {self.address} | {self.explorer}{hash.hex()} transaction failed!')
                    return False
